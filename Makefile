...
black:
	black --version
	black --config pyproject.toml examples hsa_elastica

.PHONY: black-check
black-check:
	black --version
	black --diff --check --config pyproject.toml examples hsa_elastica

.PHONY: flake8
flake8:
	flake8 --version
	flake8 hsa_elastica tests
...
.PHONY: test_coverage
test_coverage:
	pytest --cov=hsa_elastica

.PHONY: test_coverage_xml
test_coverage_xml:
	pytest --cov=hsa_elastica --cov-report=xml
